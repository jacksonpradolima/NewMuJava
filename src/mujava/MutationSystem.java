...
import mujava.enums.operator.ClassMutationOperator;
import mujava.enums.operator.ExceptionRelatedMutationOperator;
import mujava.enums.operator.TraditionalMutationOperator;
 ...
 * operators; and clean up previously generated mutants </p>
 *
 * <p>
 * Requires: mujava.config must be stored in MuJava system home directory </p>
 * <p>
...
public class MutationSystem extends OJSystem {
     ...
     */
    public static String[] cm_operators = Arrays.stream(ClassMutationOperator.values()).map(Enum::name).toArray(String[]::new);

    /**
     * List of names of traditional mutation operators
     */
    public static String[] tm_operators = Arrays.stream(TraditionalMutationOperator.values()).map(Enum::name).toArray(String[]::new);

    /**
     * List of names of exception-related mutation operators
     */
    public static String[] em_operators = Arrays.stream(ExceptionRelatedMutationOperator.values()).map(Enum::name).toArray(String[]::new);
    ...
    /**
     * Examine if type is of primitive types (boolean, byte, char, short, int,
     * long, double, void)
     */
    ...
    public static void deleteDirectory() throws IOException {
        // Delete all files and folders inside "result/"
        FileUtils.deleteDirectory(new File(MutationSystem.MUTANT_HOME + "/" + MutationSystem.DIR_NAME + "/"));
    }
    ...
    public static void setJMutationStructure() {
        try {
            ...
            String home_path = ConfigurationUtils.muJavaHomePath;
    ...
    * 
     ...
     * from user directly </p>
     */
    public static void setJMutationStructureAndSession(String home_path, String sessionName) {
        SYSTEM_HOME = home_path + "/" + sessionName;
        SRC_PATH = SYSTEM_HOME + "/src";
        CLASS_PATH = SYSTEM_HOME + "/classes";
        MUTANT_HOME = SYSTEM_HOME + "/result";
        TESTSET_PATH = SYSTEM_HOME + "/testset";
    }

    /**
     * Create the folders inside result/ - Mutants path
     *
     * @param file_name
     */
    public static void setMutationSystemPathFor(String file_name) {
        try {
            String temp;
            ...
            String mutant_dir_path = MutationSystem.MUTANT_HOME + "/" + MutationSystem.DIR_NAME;
            File mutant_path = new File(mutant_dir_path);
            mutant_path.mkdir();

            MutationSystem.CLASS_MUTANT_PATH = mutant_dir_path + "/" + MutationSystem.CM_DIR_NAME;
            File class_mutant_path = new File(MutationSystem.CLASS_MUTANT_PATH);
            class_mutant_path.mkdir();

            MutationSystem.TRADITIONAL_MUTANT_PATH = mutant_dir_path + "/" + MutationSystem.TM_DIR_NAME;
            File traditional_mutant_path = new File(MutationSystem.TRADITIONAL_MUTANT_PATH);
            traditional_mutant_path.mkdir();

            MutationSystem.ORIGINAL_PATH = mutant_dir_path + "/" + MutationSystem.ORIGINAL_DIR_NAME;
            File original_path = new File(MutationSystem.ORIGINAL_PATH);
            original_path.mkdir();
        } catch (Exception e) {
    ...
    public static void setMutationSystemPathForCLI(String file_name) {
        try {
            String temp;
            temp = file_name.substring(0, file_name.length() - ".java".length());
            temp = temp.replace('/', '.');
