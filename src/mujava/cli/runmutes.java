...
import mujava.util.ConfigurationUtils;
...
public class runmutes {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ConfigurationUtils.loadCLIConfig();
        muJavaHomePath = ConfigurationUtils.muJavaHomePath;
        ...
        // Lin 05232015
        if (jct.getTimeout() == -1) // -1 means there is no input for timeout
        // then do nothing, just use the default one
        {

        } else { // if there IS an option for timeout
            timeout_sec = 1000 * jct.getTimeout();
            if (timeout_sec <= 0) // if not a valid timeout, make it 3000
            {
                timeout_sec = 3000;
            }
        }
        ...
        if (jct.getParameters().size() == 1) {
            ...
            File folder = new File("C:\\Users\\Jackson Lima\\Documents\\Bitbucket\\muJava\\TestMuJava2\\testset");
            File[] listOfFiles = folder.listFiles();
...
//        }
        ...
        MutationSystem.setJMutationStructureAndSession(muJavaHomePath, testSessionName);
    ...
    static void runTests(String targetClassName, String testSetName, String[] mutantTypes, double percentage,
        ...
        if (!methodNameList.contains("mutant_list") || mode.equals("fresh")) {
            ...
            TestResultCLI result = test_engine.runMutants(test_result, "All method");
            result.outputToFile("All method");
            return;
        }
        ...
        // if m is dead, t not run on it.
        {
            ...
            // eliminate mutants of other types not listed
            tr.live_mutants = trimLiveMutants(tr.live_mutants, mutantTypes); 
    ...
    public static void saveTestResults(String targetClassName, Map<String, String> finalTestResults,
        ...
        if (!mode.equals("fresh")) // default mode or dead mode, no time stamp
        {
            ...
            for (Map.Entry<String, String> entry : finalMutantResults.entrySet()) {
                ...
                if (oldResults.containsKey(entry.getKey())) // if new result have overlap
                {
                    // get old results
                    ...
                    for (int i = 0; i < oldResult.size() - 2; i++) // for (String result : oldResult)
                    {
                        if (oldResult.get(i).equals("1")) {
                            sum++;
                        }
                    ...
                    for (int i = 0; i < newEntryVal.size() - 2; i++) // for (String result : oldResult)
                    {
